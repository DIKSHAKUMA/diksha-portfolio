...
const visibleHistory = computed(() => {
  return isExpanded.value ? props.history : props.history.slice(0, 2)
})
...
<template>
  <section class="client-history" v-if="history && history.length > 0">
    <div class="client-history__header">
      ...
      <p class="client-history__subtitle">{{ history.length }} project{{ history.length > 1 ? 's' : '' }} completed</p>
    </div>

    <div class="client-history__content" ref="historySection">
      <div class="client-history__timeline">
        <div v-for="(item, index) in visibleHistory" :key="index" class="client-history__item">
          <div class="client-history__item-dot"></div>
          <div class="client-history__item-content">
            <div class="client-history__item-header">
              <h6 class="client-history__project-name">{{ item.projectName }}</h6>
              <span class="client-history__year">{{ item.year }}</span>
            </div>
      ...
      <button v-if="hasMoreItems" @click="toggleExpanded" class="client-history__toggle">
        {{ isExpanded ? 'Show less' : `Show ${history.length - 2} more` }}
        <svg class="client-history__toggle-icon" :class="{ 'client-history__toggle-icon--expanded': isExpanded }"
